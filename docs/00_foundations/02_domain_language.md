...
### Ticket
A **Ticket** is the **universal work unit** across support, project delivery, and internal contexts.

Rules:
- All person work activity must be tied to a Ticket.
- Support work, project deliverables, and internal tasks all use the same Ticket entity.
- Context is determined by `lifecycle_owner` / `primary_container`, not separate tables.
- Tickets carry immutable sold baseline fields (`sold_minutes`, `sold_value_cents`) when created from accepted quotes.
- SLA applicability is explicit (support context).
- Tickets may be structured as WBS trees via `parent_ticket_id` / `root_ticket_id`.
