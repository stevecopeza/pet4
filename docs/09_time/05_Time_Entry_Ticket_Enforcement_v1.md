...
- If ticket is `is_rollup=1`: hard error (must log against leaf ticket).

Note: An unsplit sold ticket (is_rollup=0) is a leaf and accepts time directly, regardless of `is_baseline_locked` status. The only check is whether the ticket has children.
